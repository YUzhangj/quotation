...
const tab_bd_others = {
  render(versionData) {
    ...
    const eRows = eSum.final_price_usd ? `
      <tr>
        <td>电子件 (PCBA)</td>
        <td class="num">${formatNumber(eSum.final_price_usd, 4)} USD</td>
        <td class="num">${formatNumber(electronicHkd, 4)}</td>
        <td>—</td>
      </tr>
    ` : '';
    ...
    const eSummaryBlock = eSum.total_cost ? `
      <div class="data-table-wrap" style="margin-top:16px">
        <table class="data-table">
          ...
          <tbody>
            ${eSum.parts_cost != null ? `<tr><td>零件成本</td><td class="num">${formatNumber(eSum.parts_cost, 4)} USD</td></tr>` : ''}
            ${eSum.bonding_cost != null ? `<tr><td>邦定成本</td><td class="num">${formatNumber(eSum.bonding_cost, 4)} USD</td></tr>` : ''}
            ${eSum.smt_cost != null ? `<tr><td>贴片成本</td><td class="num">${formatNumber(eSum.smt_cost, 4)} USD</td></tr>` : ''}
            ${eSum.labor_cost != null ? `<tr><td>人工成本</td><td class="num">${formatNumber(eSum.labor_cost, 4)} USD</td></tr>` : ''}
            ${eSum.test_cost != null ? `<tr><td>测试费用</td><td class="num">${formatNumber(eSum.test_cost, 4)} USD</td></tr>` : ''}
            ${eSum.total_cost != null ? `<tr><td><b>合计成本</b></td><td class="num"><b>${formatNumber(eSum.total_cost, 4)} USD</b></td></tr>` : ''}
            ${eSum.profit_margin != null ? `<tr><td>利润率</td><td class="num">${formatNumber(eSum.profit_margin * 100, 1)}%</td></tr>` : ''}
            ${eSum.final_price_usd != null ? `<tr><td><b>含利润价 USD</b></td><td class="num"><b>${formatNumber(eSum.final_price_usd, 4)}</b></td></tr>` : ''}
            ${eSum.pcb_mold_cost_usd != null ? `<tr><td>PCB模费 USD</td><td class="num">${formatNumber(eSum.pcb_mold_cost_usd, 4)}</td></tr>` : ''}
          </tbody>
        </table>
      </div>
    ` : '';
